...
# ------------------------------------------------------------
# v65.1 keeps the scientific/diagnostic pipeline unchanged while
# allowing researchers to configure local installation paths.
# Environment variables are optional; the defaults preserve the
# original v65 directory layout for backward compatibility.

ENDOMLLM_ROOT = Path(
    os.environ.get(
        "ENDOMLLM_ROOT",
        str(Path.home() / "EndoMLLM"),
    )
).expanduser().resolve()

# Frozen vision pipeline lives in the configured EndoMLLM root.
sys.path.insert(0, str(ENDOMLLM_ROOT))
...
KB = Path(
    os.environ.get(
        "ENDOMLLM_KB_ROOT",
        str(ENDOMLLM_ROOT / "phase2_knowledge"),
    )
).expanduser().resolve()
...
OUTPUT_DIR = Path(
    os.environ.get(
        "ENDOMLLM_OUTPUT_DIR",
        str(KB / "evaluation/phase4_outputs"),
    )
).expanduser().resolve()
...
OPGAGENT_ROOT = Path(
    os.environ.get(
        "OPGAGENT_ROOT",
        str(Path.home() / "OPGAgent"),
    )
).expanduser().resolve()

OPGAGENT_PYTHON = Path(
    os.environ.get(
        "OPGAGENT_PYTHON",
        str(Path.home() / "opgagent_env/bin/python"),
    )
).expanduser().resolve()
OPGAGENT_TVEM = OPGAGENT_ROOT / "api_service/tvem"
OPGAGENT_YOLO = OPGAGENT_ROOT / "api_service/yolo_enumeration"
OPGAGENT_OUTPUT_DIR = Path(
    os.environ.get(
        "OPGAGENT_OUTPUT_DIR",
        str(KB / "evaluation/opgagent_outputs"),
    )
).expanduser().resolve()
